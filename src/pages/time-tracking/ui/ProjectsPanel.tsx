...
import { MOCK_PROJECTS } from '../model/constants'
...
function StatusDropdown({
  statusFilter, filtered, onSelect,
}: { statusFilter: ProjectStatus | ''; filtered: number; onSelect: (s: ProjectStatus | '') => void }) {
  ...
  const label = statusFilter ? `${STATUS_LABEL[statusFilter]} (${filtered})` : `Все проекты (${filtered})`
  return (
    <div ref={ref} className="pp__status-wrap">
      ...
      {open && (
        <div className="pp__status-dropdown">
          <button type="button" className={`pp__status-opt${!statusFilter ? ' pp__status-opt--on' : ''}`}
            onClick={() => { onSelect(''); setOpen(false) }}>
            {!statusFilter && <IcoCheck />} Все проекты ({MOCK_PROJECTS.length})
          </button>
...
function FilterDropdown({ label, options, value, onChange }: {
  ...
  return (
    <div ref={ref} className="pp__filter-wrap">
      <button type="button" className={`pp__filter-btn${active ? ' pp__filter-btn--active' : ''}`}
      ...
      {open && (
        <div className="pp__filter-menu">
          <button type="button" className={`pp__filter-opt${!value ? ' pp__filter-opt--on' : ''}`}
...
export function ProjectsPanel() {
  ...
  const clients  = useMemo(() => [...new Set(MOCK_PROJECTS.map(p => p.client))].sort(), [])
  const managers = useMemo(() => {
    const all = MOCK_PROJECTS.flatMap(p => p.managers ?? [])
  ...
  useEffect(() => {
    ...
    const h = (e: MouseEvent) => {
      if (actionRef.current && !actionRef.current.contains(e.target as Node)) setActionOpen(null)
    }
    document.addEventListener('mousedown', h)
    return () => document.removeEventListener('mousedown', h)
  }, [actionOpen])
  ...
  function toggleCollapse(client: string) {
    setCollapsed(prev => { const n = new Set(prev); n.has(client) ? n.delete(client) : n.add(client); return n })
  }
...
<div className="pp__topbar">
        ...
        <div className="pp__topbar-right">
          <FilterDropdown label="По клиенту"   options={clients}  value={clientFilter}  onChange={setClientFilter} />
          <FilterDropdown label="По менеджеру" options={managers} value={managerFilter} onChange={setManagerFilter} />
          <button type="button" className="pp__new-btn">
...
<div className="pp__thead">
            <span className="pp__th pp__th--check"><span className="pp__checkbox" /></span>
...
{grouped.length === 0 && (
            <div className="pp__empty">
              <IcoFolder />
              <span>Нет проектов по выбранным фильтрам</span>
            </div>
          )}
...
{!isCollapsed && projects.map(p => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => navigate(getProjectDetailUrl(p.id))}
...
<span className="pp__td pp__td--name">
                        <AnimatedLink className="pp__proj-name pp__proj-name--link" to={getProjectDetailUrl(p.id)}>
                        ...
                        {rem != null ? (
                          <>
                            <span className="pp__rem-val">
                              {over ? '−' : ''}{fmtAmt(Math.abs(rem), p.currency)}
                            </span>
                            {pct != null && (
...
<span className="pp__td pp__td--actions" onClick={e => e.stopPropagation()}>
                        <div className="pp__actions-wrap" ref={isActOpen ? actionRef : undefined}>
                          ...
                          {isActOpen && (
                            <div className="pp__actions-menu">
                              <button type="button" className="pp__actions-item" onClick={() => setActionOpen(null)}>Редактировать</button>
                              <button type="button" className="pp__actions-item" onClick={() => setActionOpen(null)}>Открыть</button>
                              ...
                              <button type="button" className="pp__actions-item pp__actions-item--danger" onClick={() => setActionOpen(null)}>Удалить</button>
                            </div>
                          )}
                        </div>
                      </span>
